...
mat getSphereMove(double r, vec origin){
  int gridSize = 300; 
...
//cout << r << endl;
    for(int j = 0; j < theta.n_rows;j++){
      ...
      dome.row(j) = r*buffvec.t() + origin.t() ; // + aveX.t(); // MAIN!
    }
//----------------------------------------------------------------------
...
///cout << "aaa" << endl;
//------------------------------------------------------------------------
///  for(int j = 0; j < dome.n_rows ;j++){
///    if(dome(j,0) < 1e6){
///             cout <<  dome(j,0) << " " <<  dome(j,1) << " " <<
///             dome(j,2) <<  endl;
///    }
///  }
//----------------------------------------------------------------------
