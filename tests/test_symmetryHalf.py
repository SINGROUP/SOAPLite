...
features = soapPyHalf.get_soap_structure(atoms, myAlphas, myBetas, rCut=10.0, NradBas=5, Lmax=9,crossOver=True) 
orig_atoms = atoms.copy()
# rotation check
for rotation in ['x', 'y', 'z']:
    print("rotating in", rotation)
    atoms.rotate(45, rotation)
    rot_features = soapPyHalf.get_soap_structure(atoms, myAlphas, myBetas, rCut=10.0, NradBas=5, Lmax=9,crossOver=True) 
...
for translation in [[1.0, 1.0, 1.0], [-5.0, 5.0, -5.0], [1.0, 1.0, -10.0],]:
    ...
    trans_features = soapPyHalf.get_soap_structure(atoms, myAlphas, myBetas, rCut=10.0, NradBas=5, Lmax=9,crossOver=True) 
