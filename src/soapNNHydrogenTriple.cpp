...
int main(int argc, char** argv) {
  ...
  // Flexible variables:
  mat hydrogenPos;
  hydrogenPos.load(argv[5]); // [0.1 - 5.0] EXP. BEST = 0.5 5 3 5
  double ao = atof(argv[6]); // [0.1 - 5.0] EXP. BEST = 0.5 5 3 5
  double rcut = atof(argv[7]); // [7.0 - 100.0] -> make sure to use fine grid with large cut
  int radialN = atoi(argv[8]); // [1-4]
  int lMax = atoi(argv[9]) ; // [1-9]
  int grid = atoi(argv[10]) ; // [1-9]
