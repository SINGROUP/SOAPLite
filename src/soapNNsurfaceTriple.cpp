...
int main(int argc, char** argv) {
  ...
  // Flexible variables:
  double ao = atof(argv[5]); // [0.1 - 5.0] EXP. BEST = 0.5 5 3 5
  double rcut = atof(argv[6]); // [7.0 - 100.0] -> make sure to use fine grid with large cut
  int radialN = atoi(argv[7]); // [1-4]
  int lMax = atoi(argv[8]) ; // [1-9]
  int grid = atoi(argv[9]) ; // [1-9]
...
//----Holding memory places.----
  ...
  vec typeC = zeros<vec>(coord.n_rows);

  coord = posAve(coord); 

  mat coordReload = coord;
//cout << "CAA" << endl;  
  uvec q =  getSurf(coord,atof(argv[9]));
...
//Finding where atom A and atom B are in .xyz.
  ...
  mat coord_c = zeros<mat>(sum(typeC) + 1,3);
...
//  coord(bigI,0) = -1000000;   coord(bigI,1) = -1000000;   coord(bigI,2) = -1000000; 
  ...
  newJ = 0;
  for(int i=0; i < coord.n_rows; i++)  { 
     if(type[i] == argv[4] ){coord_c.row(newJ) = coord.row(i); newJ++;}
   }
...
// Not adding Hydrogen at the end, but adding the loop position of atoms.
  coord_a.row(coord_a.n_rows - 1) = coordReload.row(q(bigI));
  coord_b.row(coord_b.n_rows - 1) = coordReload.row(q(bigI));
  coord_c.row(coord_c.n_rows - 1) = coordReload.row(q(bigI));
...
// Gaussian Smearing at Gauss-Lgedendre quadratue points
  ...
  for(int n=0; n < radialN; n++) {
    Cbuff=rsc*integ3Dvec3Atom(intMea,intMeb,intMec, intAll, getTMat(n,0,0,g,Y0,R,The,Phi));
...
for(int l = 1; l <= lMax; l++){
  for(int m=-l; m <= l; m++) {
    globalI++;
    for(int n=0; n < radialN; n++) {
        if(l == 1){Cbuff=rsc*integ3Dvec3Atom(intMea,intMeb,intMec, intAll, getT(n,l,m,g,Y1,R,The,Phi));}
        else if(l == 2){Cbuff=rsc*integ3Dvec3Atom(intMea,intMeb,intMec, intAll, getT(n,l,m,g,Y2,R,The,Phi));}
        else if(l == 3){Cbuff=rsc*integ3Dvec3Atom(intMea,intMeb,intMec, intAll, getT(n,l,m,g,Y3,R,The,Phi));}
        else if(l == 4){Cbuff=rsc*integ3Dvec3Atom(intMea,intMeb,intMec, intAll, getT(n,l,m,g,Y4,R,The,Phi));}
        else if(l == 5){Cbuff=rsc*integ3Dvec3Atom(intMea,intMeb,intMec, intAll, getT(n,l,m,g,Y5,R,The,Phi));}
        else if(l == 6){Cbuff=rsc*integ3Dvec3Atom(intMea,intMeb,intMec, intAll, getT(n,l,m,g,Y6,R,The,Phi));}
        else if(l == 7){Cbuff=rsc*integ3Dvec3Atom(intMea,intMeb,intMec, intAll, getT(n,l,m,g,Y7,R,The,Phi));}
        else if(l == 8){Cbuff=rsc*integ3Dvec3Atom(intMea,intMeb,intMec, intAll, getT(n,l,m,g,Y8,R,The,Phi));}
        else if(l == 9){Cbuff=rsc*integ3Dvec3Atom(intMea,intMeb,intMec, intAll, getT(n,l,m,g,Y9,R,The,Phi));}
