...
double* get_AlphaBeta(double* alphas, double* betas, int Nsize){
  ...
  if(l>0){
    for(int k = 0; k < Nsize; k++){
      ...
      oneO1alphaSqrt2 
      oneO1alphaSqrtX = oneO1alphaSqrt*oneO1alpha*oneO1alphaSqrtX;
      for(int n = 0; n < Nsize; n++){
...
//================================================================
int getC(double* C, double* x, double* y, double* z,double* r2, double* ReIm2, double* ReIm3, double* ReIm4, double* ReIm5, double* ReIm6,double* ReIm7, double* ReIm8, double* ReIm9, double* bOa, double* aOa, int Asize, int Nsize, int Ntypes,int posI, int typeJ){
  if(Asize == 0){
    return 0;
  }
  ...
  double sumMe = 0;
  int NsNs = Nsize*Nsize;
  int NsNsJ = NsNs*Ntypes*typeJ;
  int NsNsNtI = NsNs*Ntypes*posI;

  for(int k = 0; k < Nsize; k++){
    sumMe = 0;
    for(int i = 0; i < Asize; i++){
      sumMe += exp(aOa[k]*r2[i]); 
    }
    for(int n = 0; n < Nsize; n++){
      C[NsNsNtI + NsNsJ + n*Nsize + k] = bOa[n*Nsize + k]*sumMe;
    }
  }
...
int main(){
  ...
  for(int i = 0; i < Hsize; i++){
    for(int j = 0; j < Ntypes[0]; j++){
      ...
      getC(cn, x, y, z,r2, alphas, betas, Asize, Nsize,Ntypes[0],i,j);
    }
  }
