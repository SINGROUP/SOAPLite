...
for i, atoms in zip(np.arange(len(traj)),traj):
    ...
    soap_array = get_nnsoap(cluster, first_shell, myAlphas, myBetas, RCUT, NMAX, LMAX, all_atomtypes=[29,79])
    ...
    soapmatrix[i,:] = soap_array = get_nnsoap(cluster, first_shell, myAlphas, myBetas, RCUT, NMAX, LMAX, all_atomtypes=[29,79])
    print("")

# infos
print("shape", soapmatrix.shape)
np.save(rootname + "_nn_soap_N" + str(NMAX) + "L" + str(LMAX) + "R" + str(RCUT) + ".npy", soapmatrix)
